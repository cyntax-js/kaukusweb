...
import type { BrokerConfig, BrokerService, BrokerTemplate, BrokerTheme } from "@/broker-theme/config";
import { templatePresets, defaultBrokerConfig, registerBrokerConfig } from "@/broker-theme/config";
import { indexedDBStateStorage } from "@/lib/indexedDBStorage";

const STORAGE_KEY = 'kaukus_broker_deployment_draft';
...
const SERVICE_OPTIONS: ServiceOption[] = [
  { id: 'spot', name: 'Spot Trading', description: 'Buy and sell assets at current market prices', price: 299 },
  { id: 'futures', name: 'Futures Trading', description: 'Trade contracts for future delivery', price: 499 },
  { id: 'options', name: 'Options Trading', description: 'Trade options contracts', price: 399 },
  { id: 'private_markets', name: 'Private Markets', description: 'Access to private market offerings', price: 599 },
];
  ...
  persist(
    (set, get) => ({
      ...
      getPreviewUrl: (page = "") => {
        ...
        const url = new URL(`${window.location.protocol}//${window.location.host}/preview${pageRoute}`);
