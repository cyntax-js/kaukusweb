/**
 * OTC DESK DETAIL PAGE - Light, minimalist trading interface
...
const mockOfferData: DealerOffer = {
  ...
  description:
    "Premier Nigerian cement manufacturer with operations across Africa. Strong fundamentals and consistent dividend payouts make this an attractive investment for institutional investors.",
  side: "sell",
  quantity: 500000,
...
const mockRecentTrades = [
  { id: "1", side: "buy", price: 247.5, quantity: 75000, time: "2h ago" },
  { id: "2", side: "sell", price: 248.0, quantity: 50000, time: "5h ago" },
  { id: "3", side: "buy", price: 246.75, quantity: 100000, time: "8h ago" },
  { id: "4", side: "sell", price: 249.0, quantity: 25000, time: "12h ago" },
  { id: "5", side: "buy", price: 247.0, quantity: 80000, time: "1d ago" },
];
...
const OTCDexDetail: React.FC = () => {
  ...
  const [quantity, setQuantity] = useState<string>(
    mockOfferData.minQuantity.toString()
  );
  const [proposedPrice, setProposedPrice] = useState<number>(
    mockOfferData.pricePerUnit
  );
  ...
  const [negotiationStatus, setNegotiationStatus] = useState<
    "idle" | "negotiating" | "accepted" | "rejected"
  >("idle");
  const [isProcessing, setIsProcessing] = useState(false);
  const [negotiationMessage, setNegotiationMessage] = useState("");
  const [dealerCounterPrice, setDealerCounterPrice] = useState<number | null>(
    null
  );
  ...
  const priceDiscount = useMemo(() => {
    const diff =
      ((mockOfferData.pricePerUnit - proposedPrice) /
        mockOfferData.pricePerUnit) *
      100;
    return diff;
  }, [proposedPrice]);
  ...
  const handleStartNegotiation = async () => {
    if (parseInt(quantity) < mockOfferData.minQuantity) {
      toast.error(
        `Minimum quantity is ${mockOfferData.minQuantity.toLocaleString()} units`
      );
      return;
    }
    ...
    toast.success("Negotiation started!", {
      description: `Your offer of ${formatPrice(
        proposedPrice
      )} has been sent to the dealer`,
    });

    // Simulate dealer response after delay
    setTimeout(() => {
      const dealerPrice =
        proposedPrice + (mockOfferData.pricePerUnit - proposedPrice) * 0.6;
      ...
      const dealerRound: NegotiationRound = {
        ...
        timestamp: new Date(),
        message:
          "This is our best price for this volume. We can offer slightly better terms for larger orders.",
      };
    ...
    setTimeout(() => {
      ...
      if (Math.abs(newDealerPrice - proposedPrice) < 1) {
        ...
        const dealerRound: NegotiationRound = {
          ...
          timestamp: new Date(),
          message:
            "We can meet you halfway. This is a competitive price for this asset.",
        };
        setNegotiationRounds((prev) => [...prev, dealerRound]);
      }
    }, 2500);
  ...
  const handleAcceptPrice = async () => {
    ...
    toast.success("Trade executed!", {
      description: `Bought ${parseInt(quantity).toLocaleString()} ${
        mockOfferData.symbol
      } at ${formatPrice(dealerCounterPrice || proposedPrice)}`,
    });
  };
  ...
  const handleDirectBuy = async () => {
    ...
    toast.success("Order placed!", {
      description: `Bought ${parseInt(quantity).toLocaleString()} ${
        mockOfferData.symbol
      } at ${formatPrice(mockOfferData.pricePerUnit)}`,
    });
  };

  const availabilityPercent =
    (mockOfferData.availableQuantity / mockOfferData.quantity) * 100;
            ...
            <div>
              <div className="flex items-center gap-3">
                ...
                <span className="text-2xl font-bold text-gray-900">
                  {formatPrice(mockOfferData.pricePerUnit)}
                </span>
                <Badge
                  variant="outline"
                  className={cn(
          ...
          <div className="flex items-center gap-4 text-sm text-gray-600">
            <div>
              ...
              <span className="ml-2 font-medium">
                {formatVolume(mockOfferData.volume24h)}
              </span>
              <span className="text-emerald-600 ml-1">
                +{mockOfferData.volumeChange}%
              </span>
            </div>
            <div className="border-l border-gray-300 pl-4">
              <span className="text-gray-500">Total Vol.</span>
              <span className="ml-2 font-medium">
                {formatVolume(mockOfferData.totalVolume)}
              </span>
            </div>
            <div className="border-l border-gray-300 pl-4">
              <span className="text-gray-500">Available</span>
              <span className="ml-2 font-medium">
                {mockOfferData.availableQuantity.toLocaleString()}
              </span>
            </div>
            <div className="border-l border-gray-300 pl-4 flex items-center gap-2">
              ...
              <span>
                {Math.ceil(
                  (mockOfferData.expiresAt.getTime() - Date.now()) / 86400000
                )}
                d left
              </span>
            </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
                ...
                <CardContent className="space-y-6">
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    ...
                    <div className="p-4 bg-gray-50 rounded-lg">
                      ...
                      <p className="font-semibold text-gray-900">
                        {mockOfferData.quantity.toLocaleString()} units
                      </p>
                    </div>
                    <div className="p-4 bg-gray-50 rounded-lg">
                      <span className="text-sm text-gray-500">
                      ...
                      <p className="font-semibold text-gray-900">
                        {mockOfferData.minQuantity.toLocaleString()} units
                      </p>
                    </div>
                    <div className="p-4 bg-gray-50 rounded-lg">
                      <span className="text-sm text-gray-500">Negotiable</span>
                      <p className="font-semibold text-gray-900">
                        {mockOfferData.isBargainable ? "Yes" : "No"}
                      </p>
                    </div>
                  </div>
                </CardContent>
                ...
                <CardContent className="space-y-6">
                  <div className="grid md:grid-cols-3 gap-4">
                    ...
                    <div className="p-4 bg-gray-50 rounded-lg text-center">
                      <div className="text-2xl font-bold text-gray-900 mb-1">
                        {mockOfferData.totalTrades.toLocaleString()}
                      </div>
                      <span className="text-sm text-gray-500">
                        Total Trades
                      </span>
                  ...
                  <div className="flex items-center gap-2 p-4 bg-emerald-50 rounded-lg">
                    ...
                    <span className="text-sm text-emerald-700">
                      Verified dealer with SEC Nigeria license
                    </span>
                  </div>
                </CardContent>
              </Card>
          ...
          <div className="space-y-4">
            ...
            <Card className="bg-white border border-gray-200 rounded-xl shadow-sm">
              <CardContent className="p-4">
                ...
                <div className="flex items-center justify-between text-xs text-gray-500">
                  <span>
                    {mockOfferData.availableQuantity.toLocaleString()} available
                  </span>
            ...
            <Card className="bg-white border border-gray-200 rounded-xl shadow-sm">
              <CardHeader className="pb-2">
                <CardTitle className="text-lg flex items-center gap-2">
                  {mockOfferData.side === "sell" ? "Buy" : "Sell"}{" "}
                  {mockOfferData.symbol}
              ...
              <CardContent className="space-y-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Min: {mockOfferData.minQuantity.toLocaleString()} units
                  </p>
                </div>
                ...
                negotiationStatus !== "accepted" ? (
                  <>
                    <div>
                      <div className="flex items-center justify-between mb-2">
                        ...
                        {priceDiscount > 0 && (
                          <span className="text-xs text-emerald-600">
                            -{priceDiscount.toFixed(1)}% discount
                          </span>
                        )}
                      </div>
                      <Input
                      ...
                      <div className="flex justify-between text-xs text-gray-500 mt-1">
                        <span>
                          {formatPrice(mockOfferData.pricePerUnit * 0.9)}
                        </span>
                        <span className="text-gray-900 font-medium">
                          List: {formatPrice(mockOfferData.pricePerUnit)}
                        </span>
                        <span>
                          {formatPrice(mockOfferData.pricePerUnit * 1.05)}
                        </span>
                      </div>
                    </div>
                          ...
                          negotiationStatus === "negotiating" && (
                            <div className="mt-3 p-3 bg-amber-50 rounded-lg">
                              <p className="text-sm text-amber-700 mb-2">
                                ...
                                <strong>
                                  {formatPrice(dealerCounterPrice)}
                                </strong>
                              </p>
                  ...
                  <div>
                    ...
                    <div className="text-2xl font-bold text-gray-900 mt-1">
                      {formatPrice(
                        negotiationStatus === "accepted"
                          ? dealerCounterPrice || proposedPrice
                          : mockOfferData.pricePerUnit
                      )}
                    </div>
                  </div>
              ...
              <CardContent className="p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="text-sm text-blue-700">
                    ...
                    <p>
                      Large block trades executed directly with dealers.
