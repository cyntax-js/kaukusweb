/**
 * OTC DEX MARKET PAGE - Grid layout matching Secondary Market
  ...
  const toggleWatchlist = (offerId: string) => {
    setWatchlist(prev => {
      ...
      if (newWatchlist.includes(offerId)) {
        toast.success("Added to watchlist", { description: "You'll receive alerts for price changes" });
      } else {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50/80">
      ...
      <main className="flex-1 max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8 w-full">
        {/* Page Title */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900">OTC DEX</h1>
          <p className="text-gray-500 mt-1">High-volume P2P trading with institutional dealers</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 mb-8">
          ...
          <Card className="bg-white border border-gray-200 rounded-xl shadow-sm hover:shadow-md transition-shadow duration-200">
            <CardContent className="p-4 h-40 flex flex-col items-center justify-center">
              <p className="text-xs font-medium text-gray-500 mb-1">Market Activity</p>
          ...
          <Card className="bg-white border border-gray-200 rounded-xl shadow-sm hover:shadow-md transition-shadow duration-200">
            <CardContent className="p-4 h-40 flex flex-col">
              <p className="text-xs font-medium text-gray-500 mb-3">Market Stats</p>
              <div className="flex-1 flex flex-col justify-center space-y-3">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-lg font-semibold text-gray-900">{activeDealers}</span>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-4 h-40 flex flex-col">
              ...
              <div className="flex-1 flex items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="text-xs text-gray-500">of {totalOffers} total offers</p>
                  <div className="w-full bg-gray-200 rounded-full h-1.5 mt-2">
                    <div 
                      className="bg-amber-500 h-1.5 rounded-full transition-all duration-500" 
                      style={{ width: `${(bargainableCount / totalOffers) * 100}%` }}
                    />
                  </div>
                </div>
              </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            >
              <Star className={cn("h-4 w-4 mr-2", showWatchlistOnly && "fill-current")} />
          ...
          {filteredOffers.map((offer) => (
            ...
            >
              <CardContent className="p-4">
                ...
                {/* Asset Name */}
                <p className="text-sm text-gray-600 mb-3 line-clamp-1">{offer.assetName}</p>

                {/* Price and Side */}
                <div className="flex items-center justify-between mb-3">
                  <div className="text-xl font-bold text-gray-900">{formatPrice(offer.pricePerUnit)}</div>
                  <Badge
                    variant="outline"
                    className={cn(
                      "font-medium",
                ...
                <div className="grid grid-cols-2 gap-2 text-xs mb-3">
                  <div className="bg-gray-50 rounded-lg p-2">
                    ...
                    <div className="font-semibold text-gray-900">{offer.availableQuantity.toLocaleString()}</div>
                  </div>
                  <div className="bg-gray-50 rounded-lg p-2">
                    <div className="text-gray-500">Min Order</div>
                    <div className="font-semibold text-gray-900">{offer.minQuantity.toLocaleString()}</div>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between text-xs mb-3">
                  ...
                  <div className="flex items-center gap-1">
                    <span className="font-medium text-gray-900">{formatVolume(offer.volume24h)}</span>
                    <span className={cn(
                      "flex items-center",
                      offer.volumeChange >= 0 ? "text-emerald-600" : "text-red-500"
                    )}>
                      {offer.volumeChange >= 0 ? <TrendingUp className="h-3 w-3" /> : <TrendingDown className="h-3 w-3" />}
                ...
                <div className="flex items-center justify-between pt-3 border-t border-gray-100">
                  <div className="flex items-center gap-2 text-xs">
                    ...
                    <span className="text-amber-500">★ {offer.dealerRating}</span>
                  </div>
                  {offer.isBargainable ? (
                    ...
                    <Badge variant="outline" className="text-gray-500 text-xs">Fixed Price</Badge>
                  )}
                </div>
              </CardContent>
            </Card>
