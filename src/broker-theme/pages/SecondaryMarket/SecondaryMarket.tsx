...
const mockSettlements: Settlement[] = [
  { id: "s1", symbol: "DANGCEM", name: "Dangote Cement", settlementDate: new Date(Date.now() + 2 * 24 * 60 * 60 * 1000) }, // 2 days
  { id: "s2", symbol: "MTNN", name: "MTN Nigeria", settlementDate: new Date(Date.now() + 5 * 24 * 60 * 60 * 1000) }, // 5 days
];
...
const calculateFearGreed = (tokens: TokenData[]): number => {
  const avgPriceChange = tokens.reduce((sum, t) => sum + t.priceChange, 0) / tokens.length;
  const avgVolumeChange = tokens.reduce((sum, t) => sum + t.volumeChange, 0) / tokens.length;
...
const SecondaryMarket: React.FC = () => {
  ...
  useEffect(() => {
    if (animatedMarketSeason < marketSeasonValue) {
      const timer = setTimeout(() => {
        setAnimatedMarketSeason(prev => Math.min(prev + 1, marketSeasonValue));
      }, 25);
      return () => clearTimeout(timer);
    }
            ...
            <CardContent className="p-4 h-full">
              ...
              <div className="relative flex items-center justify-center" style={{ marginBottom: '-1rem' }}>
                ...
                <div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex flex-col items-center">
                  ...
                  <p className="text-xs text-gray-500">{getFearGreedLabel(animatedFearGreed)}</p>
                </div>
              </div>
            </CardContent>
          </Card>
              ...
              {upcomingSettlement && countdowns[upcomingSettlement.id] ? (
                <div className="flex flex-1 flex-col">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <div>
                      <p className="text-sm font-medium text-gray-900">{upcomingSettlement.symbol}</p>
                      <p className="text-xs text-gray-500">{upcomingSettlement.name}</p>
                    </div>
                  </div>
                  <div className="flex-1 flex items-center justify-center">
                    <div className="text-center">
                      ...
                      <p className="text-xs text-gray-500 mt-1">until settlement</p>
                    </div>
                  </div>
                </div>
              ) : (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50/50">
                  ...
                  <th className="text-right py-4 px-5 text-xs font-medium text-gray-500 hidden xl:table-cell">Implied FDV</th>
                  <th className="text-right py-4 px-5 text-xs font-medium text-gray-500 hidden xl:table-cell">Settle Time</th>
                </tr>
              </thead>
              <tbody>
                  ...
                  >
                    <td className="py-4 px-5">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-medium text-sm text-gray-900">{token.symbol}</p>
                    ...
                    <td className="py-4 px-5 text-right">
                      <div className="flex flex-col items-end">
                        <span className="font-medium text-sm text-gray-900">{token.lastPrice.toLocaleString()}</span>
                        <span className={cn(
                          "text-xs font-medium",
                          token.priceChange > 0 ? "text-emerald-600" : token.priceChange < 0 ? "text-red-500" : "text-gray-400"
                        )}>
                    ...
                    <td className="py-4 px-5 text-right hidden md:table-cell">
                      <div className="flex flex-col items-end">
                        <span className="text-sm text-gray-700">{formatVolume(token.volume24h)}</span>
                        <span className={cn(
                          "text-xs",
                          token.volumeChange > 0 ? "text-emerald-600" : token.volumeChange < 0 ? "text-red-500" : "text-gray-400"
                        )}>
                    ...
                    <td className="py-4 px-5 text-right hidden lg:table-cell">
                      <div className="flex flex-col items-end">
                        <span className="text-sm text-gray-700">{formatVolume(token.totalVolume)}</span>
                        <span className={cn(
                          "text-xs",
                          token.totalVolumeChange > 0 ? "text-emerald-600" : token.totalVolumeChange < 0 ? "text-red-500" : "text-gray-400"
                        )}>
                    ...
                    <td className="py-4 px-5 text-right hidden xl:table-cell">
                      <span className="text-sm text-gray-700">{token.impliedFDV}</span>
                    </td>
                    <td className="py-4 px-5 text-right hidden xl:table-cell">
                      <Badge 
                        variant="secondary" 
                        className={cn(
                          "text-xs",
                          token.status === "live" && "bg-emerald-100 text-emerald-700",
                          token.status === "upcoming" && "bg-amber-100 text-amber-700",
                          token.status === "ended" && "bg-gray-100 text-gray-600"
                        )}
                      >
                        {token.settleTime}
                      </Badge>
