...
const TradingPage = () => {
  ...
  const { marketType: marketTypeParam, pair: pairParam } = useParams<{ marketType?: string; pair?: string }>();
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="grid grid-cols-12 gap-px bg-border">
        {orderBookPosition === 'left' ? (
          <>
            ...
            <div className="col-span-2 bg-card flex flex-col" style={{ height: '500px' }}>
              <OrderBook market={selectedMarket} isDerivative={selectedMarket.type === 'derivative'} />
            </div>

            {/* Chart + BuySell - Center (8 cols) */}
            <div className="col-span-8 bg-card flex flex-col" style={{ height: '500px' }}>
              <div className="flex-1 min-h-0">
                <TradingChart market={selectedMarket} />
              </div>
              <div className="border-t border-border" style={{ height: '180px' }}>
                <TradingPanel 
            ...
            <div className="col-span-2 bg-card overflow-auto" style={{ height: '500px' }}>
              <MarketsList markets={availableMarkets} selectedMarket={selectedMarket} onSelectMarket={handleSelectMarket} enabledServices={config.services} />
            </div>
          </>
        ) : (
          <>
      ...
      <div className="bg-card border-t border-border grid grid-cols-12 gap-px" style={{ minHeight: '200px' }}>
