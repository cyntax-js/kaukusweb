...
const mockPrivateMarketHoldings = [
  ...
  },
  {
    id: "pm4",
    asset: "LAFARGE-CP",
    name: "Lafarge Africa Commercial Paper",
    units: 50,
  ...
  },
  {
    id: "pm5",
    asset: "NTB-91D",
    name: "Nigerian Treasury Bill 91-Day",
    units: 200,
    investedValue: 196000,
    currentValue: 200000,
    yield: 8.2,
    maturity: "Apr 2025",
    type: "T-Bill",
...
const PortfolioPage = () => {
  ...
  const cashHolding = holdings.find(
    (h) => h.asset === "USDT" || h.asset === "USD" || h.asset === "NGN"
  );
  ...
  const privateMarketValue = mockPrivateMarketHoldings.reduce(
    (sum, h) => sum + h.currentValue,
    0
  );
  const privateMarketInvested = mockPrivateMarketHoldings.reduce(
    (sum, h) => sum + h.investedValue,
    0
  );
  const privateMarketPnl = privateMarketValue - privateMarketInvested;
  const avgYield =
    mockPrivateMarketHoldings.reduce((sum, h) => sum + h.yield, 0) /
    mockPrivateMarketHoldings.length;

  // Pie chart data
  const pieData = smartAccountEnabled
    ? mockPrivateMarketHoldings.map((h) => ({
        name: h.asset,
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        Viewing private market yields, bonds, and alternative
                        investments
                      </p>
                    </div>
                  </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <Card className="bg-white border border-gray-200 rounded-xl shadow-sm">
              <CardContent className="p-5">
                ...
                >
                  {(smartAccountEnabled ? privateMarketPnl : portfolioChange) >=
                  0 ? (
                  ...
                  <span className="text-sm font-medium">
                    ...
                    {smartAccountEnabled
                      ? (
                          (privateMarketPnl / privateMarketInvested) *
                          100
                        ).toFixed(2)
