 ...
 * ============================================================
 * 
 * Registration page for broker theme users.
 * Uses AuthLayout which adapts based on broker config.
 * 
...
const SignupPage = () => {
  ...
  const { signup, isLoading, error, isAuthenticated, clearError } = useBrokerAuthStore();
  ...
  useEffect(() => {
    if (isAuthenticated) {
      navigate(`${appPrefix}/markets`);
    }
  }, [isAuthenticated, navigate, appPrefix]);
  ...
  return (
    <AuthLayout 
      ...
      subtitle={`Start trading on ${config.brokerName || 'our platform'} today`}
    >
      ...
      <p className="mt-6 text-center text-sm text-muted-foreground">
        ...
        <Link to={`${publicPrefix}/login`} className="text-primary font-medium hover:underline">
          Sign in
        </Link>
      </p>
    </AuthLayout>
